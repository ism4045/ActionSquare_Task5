...
void Tetris::UpdateCurrentBlock()
{
...
void Tetris::RemoveCurrentBlock()
{
	for (int i = 0; i < currentBlock.tileInfo.size(); i++) {
		int tileX = Pos.first + currentBlock.tileInfo[i].X;
		int tileY = Pos.second + currentBlock.tileInfo[i].Y;
...
void Tetris::CalculateInfo(int clearLineNum) {
	int temp = 40;
...
void Tetris::ReturnNormalSpeed()
{
	processSpeed = levelSpeed;
	gamePeriod = 1000.0 / processSpeed;
}

void Tetris::DoHardDrop()
{
	if (!stop) {
		RemoveCurrentBlock();
		while (CanFall(Pos))
...
bool Tetris::CanFall(pair<int, int> p)
{
...
bool Tetris::CanRotate(int& moveX, int& moveY)
{
